...
public class OsLib extends TwoArgFunction {
	...
	class OsLibFunc extends VarArgFunction {
		...
		public Varargs invoke(Varargs args) {
			try {
				...
				case DATE: {
					String s = args.optionalJString(1, "%c");
				...
				case EXECUTE:
					return execute(args.optionalJString(1, null));
				case EXIT:
					exit(args.optionalInt(1, 0));
					return NONE;
				case GETENV: {
				...
				case RENAME:
					...
					return LuaValue.TRUE;
				case SETLOCALE: {
					String s = setlocale(args.optionalJString(1,null), args.optionalJString(2, "all"));
					return s!=null? valueOf(s): NIL;
				}
				case TIME:
					return valueOf(time(args.optionalTable(1, null)));
				case TMPNAME:
	...
	protected double time(LuaTable table) {
		...
		if (table == null) {
			...
			c.set(Calendar.HOUR_OF_DAY, table.get("hour").optionalInt(12));
			c.set(Calendar.MINUTE, table.get("min").optionalInt(0));
			c.set(Calendar.SECOND, table.get("sec").optionalInt(0));
