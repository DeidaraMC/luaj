...
public class StringLib extends TwoArgFunction {
	...
	static final class _byte extends VarArgFunction {
		public Varargs invoke(Varargs args) {
			...
			int l = s.m_length;
			int posi = posrelat( args.optionalInt(2,1), l );
			int pose = posrelat( args.optionalInt(3,posi), l );
			int n,i;
			if (posi <= 0) posi = 1;
			if (pose > l) pose = l;
	...
	static final class gsub extends VarArgFunction {
		public Varargs invoke(Varargs args) {
			...
			int max_s = args.optionalInt( 4, srclen + 1 );
	...
	static final class sub extends VarArgFunction {
		public Varargs invoke(Varargs args) {
			...
			int end = posrelat( args.optionalInt( 3, -1 ), l );
